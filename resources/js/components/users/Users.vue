<template>
    <div class="mx-3">
        <div class="row">
          <card-counter title="Total" color="primary" :count="users ? users.total : 0" icon="clipboard-list" size="4"/>
          <card-counter title="Admin" color="success" :count="admin_count" icon="times-circle" size="4" />
          <card-counter title="Agentes" color="info" :count="staff_count " icon="envelope-open" size="4" />
          <card-counter title="Departamento" color="secondary" :count="department_count" icon="clipboard-list" size="4"/>
          <card-counter title="Clientes" color="dark" :count="customer_count" icon="times-circle" size="4" />
          <card-counter title="Contactos" color="danger" :count="contact_count" icon="envelope-open" size="4" />
        </div>
        ...
        <transition name="fade" v-else-if="users.data" mode="out-in">
            <users-table :users="users" @page="getUsers" :searched="searched" @deleted="succeeded"
              @edit="editUser"></users-table>
        </transition>
...
export default {
  ...
  methods: {
    ...
    getCounters() {
      axios.get('/api/get_users_counters')
        .then(res => {
